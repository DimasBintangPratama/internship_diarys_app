...
def submit_permission():
    ...
    try:
        ...
        print("Data User ID:", user_id)

        # Cek apakah sudah presensi
        cursor.execute("""
            SELECT COUNT(*) AS total FROM presensi_mahasiswa 
            WHERE nama_mahasiswa = (SELECT nama FROM mahasiswa WHERE id = %s) 
            AND DATE(waktu_presensi) = %s
        """, (user_id, today))
        presensi_today = cursor.fetchone()['total']

        if presensi_today > 0:
            print("Mahasiswa sudah presensi hari ini.")
            return jsonify({"status": "error", "message": "Anda sudah mengisi kehadiran hari ini. Permission tidak dapat diajukan."}), 400

        # Cek apakah sudah mengajukan permission
        cursor.execute("""
        ...
        if existing_permission > 0:
            print("Mahasiswa sudah mengajukan permission hari ini.")
        ...
        print("Data Form: Mulai =", mulai, ", Selesai =", selesai, ", Status =", status, ", Keterangan =", keterangan)

        # Simpan data ke database
        cursor.execute("""
            INSERT INTO permission (mahasiswa_id, tanggal_mulai, tanggal_selesai, status, keterangan, created_at)
            VALUES (%s, %s, %s, %s, %s, NOW())
        """, (user_id, mulai, selesai, status, keterangan))
        conn.commit()
